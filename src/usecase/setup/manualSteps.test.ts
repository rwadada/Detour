...
describe('manualSteps', () => {
  it('leaves setup steps unprefixed and includes both the cert-trust and proxy-config instructions', () => {
    ...
    expect(steps.every((s) => !s.message.startsWith('Verify:'))).toBe(true);
  });
  ...
  it('cleanup\'s one step is its own "Turn the proxy off" wording — never cert trust, never proxyConfig\'s "turn it on" text', () => {
    for (const target of SETUP_TARGETS) {
      const steps = manualSteps('cleanup', target, ctx);
      expect(steps).toHaveLength(1);
      expect(steps[0]!.message).toContain('Turn the proxy off');
      expect(steps[0]!.message).not.toContain('Trust the CA cert');
      expect(steps[0]!.message).not.toContain('Configure the proxy');
    }
  });
});
