...
import type { BreakpointPayload } from '@/shared/api';
import {
  decodeCapturedBody,
  encodeBodyToBase64,
...
export function BreakpointEditor({ payload }: { payload: BreakpointPayload }) {
  const resumeBreakpointRequest = useBreakpointResumeStore((s) => s.resumeBreakpointRequest);
  const resumeBreakpointResponse = useBreakpointResumeStore((s) => s.resumeBreakpointResponse);
  const abortBreakpoint = useBreakpointResumeStore((s) => s.abortBreakpoint);
