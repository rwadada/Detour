...
describe('RingBuffer', () => {
  ...
  describe('byteLimit (issue #165)', () => {
    ...
    // agy code review: the backend counterpart to this class hands upsert()
    // the *same* CapturedExchange object reference for a `request` event
    // and the `response` event that later mutates it in place, unlike this
    // test file's other cases, which always upsert a brand-new object. This
    // client-side copy happens not to hit that (every WS message is a
    // freshly-parsed JSON object), but the two copies are meant to stay
    // identical, so this proves the same fix here too.
