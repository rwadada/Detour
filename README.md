...
- `action.type: "rewrite"`: for `request`/`response` independently, adds/removes headers (`headers.set`/`headers.remove`) and rewrites the body (`body.set` replaces it wholesale, `body.replace` does string/regex substitution). `request.query` (`query.set`/`query.remove`) additionally adds/removes URL query string parameters — there's no `response.query` since a response has no URL
