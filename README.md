...
- `action.type: "script"`: runs a `beforeRequest`/`beforeResponse` hook from a CommonJS module at `path` (resolved relative to rules.json) for transformations the declarative actions above can't express. **Requires `detour start --allow-scripts`** — without it, a matching `script` rule is skipped (with a warning) rather than executed; see the security note below. Each hook is optional — a module with only one of them leaves the other phase untouched — and may be `async`:
  ...
  **SECURITY (issues #98, #161):** a `script` module is loaded with plain Node `require()` and runs as arbitrary CommonJS JavaScript with detour's own process permissions — up to and including its CA private key — so anything that code can do, so can whoever can get their rule into rules.json. Two independent restrictions apply:
  ...
  - **The dashboard's `setRules` can never add a new `script` rule or change an existing one's `path`, regardless of `--allow-scripts`.** A remote client that can reach `setRules` (over the network, behind nothing but a dashboard password — or nothing at all, if none is set) could otherwise use it to inject or repoint a `script` rule and get it to run with `--allow-scripts` on. `--allow-scripts` is meant to gate a rules.json you already trust, not code delivered over the wire — a `setRules` write attempting either is rejected outright, with the reason shown in the dashboard, and never reaches disk. Editing rules.json directly (a text editor, or `detour test`/`detour record`'s own file writes) is unaffected.
